...
let product = [
 ...
 let vid=document.getElementById("view-add-data");
 let countnum =document.getElementById("count");
 let ndata=document.getElementById("view-add-data-n");
 const getdata=(res)=>JSON.parse(localStorage.getItem(res)) || [];

 let storage=getdata("cartdata");
product.forEach(data => {
    viewdata.innerHTML+=`<tr>
                <td><img src="${data.images[0]}" width="200px"></td>
                <td>${data.title}</td>
                <td>${data.description}</td>
                <td>${data.price}</td>
                <td>${data.discountPercentage}</td>
                <td>${data.rating}</td>
                <td width="150px"><button class="btn btn-primary" onclick="add(${data.id})">Add to cart</button></td>
            </tr>`
...
const add =(id)=>{
    ...
    let productdata = product.find((data) => data.id == id); 
    if(storage.some((data)=> data.id == id)){
...
const count =()=>countnum.innerHTML = storage.length;

const seedata=()=>{
    vid.innerHTML=storage.map((data)=>
    `<tr>
                <td><img src="${data.images[0]}" width="200px"></td>
                <td>${data.title}</td>
                <td>${data.description}</td>
                ...
                <td>${data.discountPercentage}</td>
                <td>${data.rating}</td>
                <td width="200px" ><a href="#" class="btn btn-danger" onclick="return dataDelete(${data.id})">delete</a></td>
            </tr>`
).join('');   
}
const dataDelete=(id)=>{
    localStorage.setItem("cartdata",JSON.stringify(storage=storage.filter((eve)=> eve.id!=id))),seedata(),count();
